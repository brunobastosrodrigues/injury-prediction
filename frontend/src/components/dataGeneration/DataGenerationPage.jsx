...
function DataGenerationPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Data Generation</h1>
        <p className="text-gray-600 dark:text-slate-400 mt-2">Generate synthetic athlete training data for injury prediction</p>
      </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                disabled={isRunning}
                className="w-full px-4 py-2.5 bg-gray-100 dark:bg-slate-800 border border-gray-300 dark:border-slate-700 rounded-xl text-gray-900 dark:text-white placeholder-slate-500 focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:bg-gray-100 dark:bg-gray-100 dark:bg-slate-800/50 disabled:text-gray-500 dark:text-slate-500 transition-all"
              />
              <p className="text-xs text-gray-500 dark:text-slate-500 mt-1.5">Between 1 and 5000</p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 bg-gray-100 dark:bg-slate-800 border border-gray-300 dark:border-slate-700 rounded-xl text-gray-900 dark:text-white placeholder-slate-500 focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:bg-gray-100 dark:bg-gray-100 dark:bg-slate-800/50 disabled:text-gray-500 dark:text-slate-500 transition-all"
              />
              <p className="text-xs text-gray-500 dark:text-slate-500 mt-1.5">For reproducibility</p>
            </div>
            ...
            <div className="pt-4 flex space-x-3">
              <button
                ...
                disabled={isRunning}
                className="flex-1 bg-gradient-to-r from-blue-600 to-blue-700 text-gray-900 dark:text-white py-2.5 px-4 rounded-xl hover:from-blue-500 hover:to-blue-600 disabled:from-slate-700 disabled:to-slate-700 disabled:text-gray-500 dark:text-slate-500 disabled:cursor-not-allowed transition-all font-medium shadow-lg shadow-blue-500/25 disabled:shadow-none flex items-center justify-center gap-2"
              >
        ...
        <Card title="Generation Progress">
          {jobStatus ? (
            <div className="space-y-4">
              ...
              {(jobStatus.status === 'completed' || jobStatus.status === 'cancelled') && (
                <button
                  ...
                  className="w-full py-2 text-sm text-gray-600 dark:text-slate-400 hover:text-gray-700 dark:text-slate-300 border border-gray-300 dark:border-slate-700 rounded-lg hover:bg-gray-100 dark:bg-slate-800 transition-colors"
                >
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 dark:text-slate-500">Configure and start generation to see progress here</p>
            </div>
          )}
        </Card>
      </div>
      ...
      <Card title="Available Datasets">
        {datasets.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-500 dark:text-slate-500">No datasets generated yet</p>
            ...
            <table className="min-w-full">
              <thead>
                <tr className="border-b border-gray-200 dark:border-slate-800">
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-slate-400 uppercase tracking-wider">Dataset ID</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-slate-400 uppercase tracking-wider">Athletes</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-slate-400 uppercase tracking-wider">Year</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-slate-400 uppercase tracking-wider">Injury Rate</th>
              ...
              <tbody className="divide-y divide-slate-800">
                {datasets.map(dataset => (
                  <tr key={dataset.id} className="hover:bg-gray-100 dark:bg-gray-100 dark:bg-slate-800/50 transition-colors">
                    <td className="px-4 py-3 font-mono text-sm text-gray-700 dark:text-slate-300">{dataset.id}</td>
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-slate-300">{dataset.n_athletes}</td>
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-slate-300">{dataset.simulation_year}</td>
