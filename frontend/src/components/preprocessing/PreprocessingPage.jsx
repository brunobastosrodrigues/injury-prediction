...
function PreprocessingPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-white">Preprocessing</h1>
        <p className="text-slate-400 mt-2">Feature engineering and train/test splitting</p>
      </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="space-y-2">
                <label className={`flex items-center p-3 rounded-xl cursor-pointer transition-all border ${
                  ...
                  />
                  <div>
                    <span className="text-sm text-white font-medium">Athlete-based</span>
                    <p className="text-xs text-slate-500">Generalize to new athletes</p>
                  </div>
                </label>
                <label className={`flex items-center p-3 rounded-xl cursor-pointer transition-all border ${
                  ...
                  />
                  <div>
                    <span className="text-sm text-white font-medium">Time-based</span>
                    <p className="text-xs text-slate-500">Predict future for known athletes</p>
                  </div>
                </label>
              </div>
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white focus:ring-2 focus:ring-blue-500 disabled:bg-slate-800/50 disabled:text-slate-500 transition-all"
              />
              <p className="text-xs text-slate-500 mt-1.5">Predict injury within this many days</p>
            </div>

            <button
              onClick={handlePreprocess}
              disabled={isRunning || !config.dataset_id}
              className="w-full bg-gradient-to-r from-purple-600 to-purple-700 text-white py-2.5 px-4 rounded-xl hover:from-purple-500 hover:to-purple-600 disabled:from-slate-700 disabled:to-slate-700 disabled:text-slate-500 disabled:cursor-not-allowed transition-all font-medium shadow-lg shadow-purple-500/25 disabled:shadow-none"
            >
        ...
        <Card title="Preprocessing Progress">
          {jobStatus ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-slate-500">Configure and run preprocessing to see progress</p>
            </div>
          )}
        </Card>
      </div>
      ...
      <Card title="Available Splits">
        {splits.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-slate-500">No preprocessed splits yet</p>
            ...
            <table className="min-w-full">
              <thead>
                <tr className="border-b border-slate-800">
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Split ID</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Strategy</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Train Samples</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Test Samples</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Features</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800">
                {splits.map(split => (
                  <tr key={split.id} className="hover:bg-slate-800/50 transition-colors">
                    <td className="px-4 py-3 font-mono text-sm text-slate-300">{split.id}</td>
                    <td className="px-4 py-3 text-sm">
                      <span className="px-2 py-1 rounded-lg bg-purple-500/10 text-purple-400 border border-purple-500/20 text-xs">
                        {split.split_strategy}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-sm text-slate-300">{split.train_samples?.toLocaleString()}</td>
                    <td className="px-4 py-3 text-sm text-slate-300">{split.test_samples?.toLocaleString()}</td>
                    <td className="px-4 py-3 text-sm text-slate-300">{split.n_features}</td>
                    <td className="px-4 py-3 text-sm">
                      <button
